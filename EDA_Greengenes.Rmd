...
fileNameIn1  <- "99_otu_taxonomy.txt.gz"
fileNameIn2  <- "data/mtx_biom_ID_all.csv" # All IDs from .biom files
dirOut1      <- "greengenes" # Where to download Greengenes data
```
...
mtx_greengenes <- read_tsv(file = paste0(dirOut1, "/", fileNameIn1), col_names = FALSE)
head(mtx_greengenes)

mtx_biom_ID_all <- read_csv(file = fileNameIn2, col_types = "c")
mtx_biom_ID_all[grepl("EP", mtx_biom_ID_all$mtx_biom_ID_all), ]
